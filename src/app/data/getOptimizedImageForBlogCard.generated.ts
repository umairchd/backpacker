...
export type BlogImageFragment = { id: string, fileName: string, blog?: string | null, regionCategoryCard?: string | null };

export type BlogImageQueryVariables = Types.Exact<{
  url: Types.Scalars['String'];
}>;


export type BlogImageQuery = { transformImage: { id: string, fileName: string, blog?: string | null, regionCategoryCard?: string | null } };

export const BlogImageFragmentDoc = gql`
    fragment BlogImage on Image {
  id
  blog: imgSrc(
    transform: {width: 585, height: 367, enlarge: true, resizeType: FILL}
  )
  regionCategoryCard: imgSrc(
    transform: {width: 624, height: 280, enlarge: true, resizeType: FILL}
  )
  fileName
}
    `;
export const BlogImageDocument = gql`
...
/**
 * __useBlogImageQuery__
 *
 * To run a query within a React component, call `useBlogImageQuery` and pass it any options that fit your needs.
 * When your component renders, `useBlogImageQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useBlogImageQuery({
 *   variables: {
 *      url: // value for 'url'
 *   },
 * });
 */
export function useBlogImageQuery(baseOptions: Apollo.QueryHookOptions<BlogImageQuery, BlogImageQueryVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<BlogImageQuery, BlogImageQueryVariables>(BlogImageDocument, options);
      }
export function useBlogImageLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<BlogImageQuery, BlogImageQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<BlogImageQuery, BlogImageQueryVariables>(BlogImageDocument, options);
        }
export type BlogImageQueryHookResult = ReturnType<typeof useBlogImageQuery>;
export type BlogImageLazyQueryHookResult = ReturnType<typeof useBlogImageLazyQuery>;
export type BlogImageQueryResult = Apollo.QueryResult<BlogImageQuery, BlogImageQueryVariables>;