...
export type ImageSliderImageFragment = { id: string, fileName: string, small1400?: string | null };

export type ImageSliderImageQueryVariables = Types.Exact<{
  url: Types.Scalars['String'];
}>;


export type ImageSliderImageQuery = { transformImage: { id: string, fileName: string, small1400?: string | null } };

export const ImageSliderImageFragmentDoc = gql`
  ...
  fileName
    ...
    `;
export const ImageSliderImageDocument = gql`
    query ImageSliderImage($url: String!) {
  transformImage: imgTransformImage(url: $url) {
    ...ImageSliderImage
  }
}
    ${ImageSliderImageFragmentDoc}`;

/**
 * __useImageSliderImageQuery__
 *
 * To run a query within a React component, call `useImageSliderImageQuery` and pass it any options that fit your needs.
 * When your component renders, `useImageSliderImageQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useImageSliderImageQuery({
 *   variables: {
 *      url: // value for 'url'
 *   },
 * });
 */
export function useImageSliderImageQuery(baseOptions: Apollo.QueryHookOptions<ImageSliderImageQuery, ImageSliderImageQueryVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<ImageSliderImageQuery, ImageSliderImageQueryVariables>(ImageSliderImageDocument, options);
      }
export function useImageSliderImageLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<ImageSliderImageQuery, ImageSliderImageQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<ImageSliderImageQuery, ImageSliderImageQueryVariables>(ImageSliderImageDocument, options);
        }
export type ImageSliderImageQueryHookResult = ReturnType<typeof useImageSliderImageQuery>;
export type ImageSliderImageLazyQueryHookResult = ReturnType<typeof useImageSliderImageLazyQuery>;
export type ImageSliderImageQueryResult = Apollo.QueryResult<ImageSliderImageQuery, ImageSliderImageQueryVariables>;